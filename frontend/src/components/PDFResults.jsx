...
const PDFResults = ({ results, reset }) => {
  ...
  // Split results into sentences when component mounts
  useEffect(() => {
    const allSentences = results.flatMap(section => 
      section.split(/[.!?]+/).filter(s => s.trim().length > 0).map(s => s.trim() + '.')
    )
  ...
  return (
    <div className="pdf-results-container">
      ...
      <div className="sentence-stack">
        ...
        {/* Spoken sentences stack (below current sentence) */}
